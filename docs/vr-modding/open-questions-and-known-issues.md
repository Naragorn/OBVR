...
| The visible body moved the view - it did not | With `[Body] Visible=1` the view lurched on every look: left or right turned it up, up and down went too far | first person, look around, 2026-09-07 | `LookControl`, not `PlayerBody.cpp` | the camera's ancestry log: "WorldRoot CameraNode < WorldRoot Node", not under the skeleton; the live INI had `BlockVerticalLook=0` written by laser clicks in OBVR's menu, which with `AimAtSource=1` doubles the pitch; `2b5ec02` forces it on under `AimAtSource` | none needed | n/a | cause found; the body itself waits for a second headset run, `Visible=0` by default |
| Snap turning did nothing | `[Look] SnapTurning=1` turned only the camera heading, rebuilt from the game's every frame | review 2026-09-25, not seen in a headset | `camera/SnapTurn.h`, `CameraHook` | code reading: `LookControl::Update` overwrote the snapped heading | none | n/a | rebuilt: the turn goes into rotZ; headset test open |
