...
void ReadRuntimeValues(Config& config, const char* path) {
	...
	config.aimBodyReturns = ReadBool("Look", "AimBodyReturns", config.aimBodyReturns, path);
	config.aimReturnSpeed = ReadFloat("Look", "AimReturnSpeed", config.aimReturnSpeed, path);
...
bool Config::Load(const char* fileName) {
	...
	OBVR_LOG("Config: AimBodyReturns=%d (%.1f) AimInThirdPerson=%d",
	         aimBodyReturns ? 1 : 0,
	         static_cast<double>(aimReturnSpeed),
