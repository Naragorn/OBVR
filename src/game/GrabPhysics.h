...
// The player's own capsule at the release. While held, the body is in the
// player's group and passes through the capsule; putting the old group back
// while the object is still inside it hands Havok an overlap, which it
// resolves by pushing the object out - the likeliest source of "a short flick
// of the wrist shoots it away" (2026-09-26), seen with OBVR's throw and
// without it alike, since a flick lets go near the chest. Not yet confirmed:
// the release log line says whether the object was inside, and the flight
// line after it what speed it left with.
//
// The capsule's radius is the one the grab update keeps its target out of
// (0x0066DF0E..0x0066DF31): the character controller of the player
// (0x0065A2C0, thiscall(actor)) asked for its shape's radius (0x008913C0,
// thiscall(controller), x87 float), times 6.999 (the double at 0x00A372E0)
// plus 5 (0x00A3F3F0), measured on the horizontal only (the z set to 0 at
// 0x0066DEEC..0x0066DF01).
inline constexpr UInt32 kControllerOf = 0x0065A2C0;
inline constexpr UInt32 kControllerRadius = 0x008913C0;
inline constexpr float kControllerRadiusToUnits = 6.999f;
inline constexpr float kControllerRadiusMarginUnits = 5.0f;

// Whether an object's bound sphere reaches into the player's capsule, on the
// horizontal: the old group waits until it does not.
inline bool InsidePlayerCapsule(const NiPoint3& centre, float radius, const NiPoint3& player,
                                float capsuleUnits) {
	const float dx = centre.x - player.x;
	const float dy = centre.y - player.y;
	const float reach = capsuleUnits + (radius > 0.0f ? radius : 0.0f);
